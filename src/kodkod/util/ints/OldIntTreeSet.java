...
import kodkod.util.ints.OldTreeSequence.Entry;
 ...
 * @author Emina Torlak
 */
final class OldIntTreeSet extends AbstractIntSet {
	/* @invariant all e: ints.entries | e.index <= e.value && 
	 *            all e': ints.entries - e | e'.value < e.index - 1 ||
	 *                                       e'.index > e.value + 1
	 */
	private final OldTreeSequence<MutableInteger> ints;
	private int size;
	 ...
	 */
	public OldIntTreeSet() {
		this.ints = new OldTreeSequence<MutableInteger>();
		this.size = 0;
	}
	 ...
	 */
	public OldIntTreeSet(Collection<? extends Integer> c) {
		this();
		addAll(c);
	}
...
//	}
	...
	public boolean containsAll(Collection<?> c) {
		if (c instanceof OldIntTreeSet) {
			OldIntTreeSet s = (OldIntTreeSet) c;
			if (size>=s.size) {
	 ...
	 */
	private boolean precedes(OldIntTreeSet s) {
...
//		System.out.println(min + ".... "  + max + " " + ints + " " + minFloor);
	...
	private final class AscendingIterator implements IntIterator {
		...
		public void remove() {
			...
			OldIntTreeSet.this.remove(lastReturned);
