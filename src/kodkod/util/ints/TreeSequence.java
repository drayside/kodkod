...
public final class TreeSequence<V> extends AbstractSparseSequence<V> {
	...
	public IndexedEntry<V> successor(int index) {
		final Entry<V> c = tree.searchGTE(index);
		return c == null || c.key>index ? c : tree.successor(c);
	}
	...
	public IndexedEntry<V> predecessor(int index) {
		final Entry<V> c = tree.searchLTE(index);
		return c == null || c.key<index ? c : tree.predecessor(c);
	}
...
//	}
	...
	private static final class Entry<V> extends IntTree.Node<Entry<V>> implements IndexedEntry<V> {
		V value;
